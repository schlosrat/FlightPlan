...
namespace MuMech
{
    public static class OrbitalManeuverCalculator
    {
        ...
        public static double GetLatitude(this PatchedConicsOrbit o, double UT)
        {
            ...
            // Test some code in NodeManager's OrbitExtensions.cs
            Vector3d thisVec = o.WorldOrbitalVelocityAtUT(UT);
            FlightPlanPlugin.Logger.LogDebug($"WorldOrbitalVelocityAtUT({UT}) = [{thisVec.x}, {thisVec.y}, {thisVec.z}]");
            thisVec = o.WorldBCIPositionAtUT(UT);
            FlightPlanPlugin.Logger.LogDebug($"WorldBCIPositionAtUT({UT}) = [{thisVec.x}, {thisVec.y}, {thisVec.z}]");
            thisVec = o.WorldPositionAtUT(UT);
            FlightPlanPlugin.Logger.LogDebug($"WorldPositionAtUT({UT}) = [{thisVec.x}, {thisVec.y}, {thisVec.z}]");
            V3 thatVec = o.RightHandedOrbitalVelocityAtUT(UT);
            FlightPlanPlugin.Logger.LogDebug($"RightHandedOrbitalVelocityAtUT({UT}) = [{thatVec.x}, {thatVec.y}, {thatVec.z}]");
            thatVec = o.RightHandedBCIPositionAtUT(UT);
            FlightPlanPlugin.Logger.LogDebug($"RightHandedBCIPositionAtUT({UT}) = [{thatVec.x}, {thatVec.y}, {thatVec.z}]");
            thisVec = o.OrbitNormal();
            FlightPlanPlugin.Logger.LogDebug($"OrbitNormal() = [{thisVec.x}, {thisVec.y}, {thisVec.z}]");
            thisVec = o.NormalPlus(UT);
            FlightPlanPlugin.Logger.LogDebug($"NormalPlus({UT}) = [{thisVec.x}, {thisVec.y}, {thisVec.z}]");
        ...
        //Computes the escape velocity for a given body.
        ...
        //If the given latitude is too large, so that an orbit with a given inclination never attains the
        //given latitude, then this function returns either 90 (if -90 < inclination < 90) or 270.
        public static double HeadingForLaunchInclination(PatchedConicsOrbit o, double inclinationDegrees)
        {
            (V3 r, V3 v) = o.RightHandedStateVectorsAtUT(GameManager.Instance.Game.UniverseModel.UniverseTime);
        ...
        public static Vector3d DeltaVToChangeInclination(PatchedConicsOrbit o, double UT, double newInclinationDeg)
        {
            ...
            V3 dv = Simple.DeltaVToChangeInclination(r, v, Deg2Rad(newInclinationDeg));
            ...
            bool coplanar = true, bool rendezvous = true, bool capture = true)
        {
            ...
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForHohmannTransfer: synodicPeriod = {FPUtility.SecondsToTimeString(synodicPeriod)}");
            ...
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForHohmannTransfer: lastApsisPhaseAngle = {lastApsisPhaseAngle}");

            double minTime = UT;
            double maxTime = UT + 1.5 * synodicPeriod;
            FlightPlanPlugin.Logger.LogDebug($"Searching for transfer window between {FPUtility.SecondsToTimeString(minTime - UT)} and {FPUtility.SecondsToTimeString(maxTime - UT)} from now");
            ...
            for (int i = 1; i <= numDivisions; i++)
            {
                ...
                FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForHohmannTransfer: apsisPhaseAngle = {apsisPhaseAngle}");
            ...
            out double burnUT)
        {
            ...
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForCheapestCourseCorrection: collisionDV = {collisionDV:N3} m/s");
            ...
            int iterCount = 0;
            soiEnterUT = (now + collisionUT) / 2.0;
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForCheapestCourseCorrection: Time to SOI = {FPUtility.SecondsToTimeString(soiEnterUT - now)} from now");
            ...
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForCheapestCourseCorrection: displacementDir        = {displacementDir:N3}");
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForCheapestCourseCorrection: interceptTarget        = {interceptTarget:N3}");
            ...
            out double burnUT)
        {
            ...
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForInterplanetaryTransferEjection: From {o.referenceBody.Name} at UT {FPUtility.SecondsToTimeString(UT)}");
            ...
            if (syncPhaseAngle)
            {
                ...
                FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForInterplanetaryTransferEjection: Calling DeltaVAndTimeForHohmannTransfer: syncPhaseAngle = {syncPhaseAngle}");
            ...
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForInterplanetaryTransferEjection: idealBurnUT: {FPUtility.SecondsToTimeString(idealBurnUT - UT)} from now");
            ...
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForInterplanetaryTransferEjection: ejectionOrbitInitialVelocity: [{ejectionOrbitInitialVelocity.x}, {ejectionOrbitInitialVelocity.y}, {ejectionOrbitInitialVelocity.z}]");
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForInterplanetaryTransferEjection: ejectionOrbitFinalVelocity: [{ejectionOrbitFinalVelocity.x}, {ejectionOrbitFinalVelocity.y}, {ejectionOrbitFinalVelocity.z}]");
            ...
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForInterplanetaryTransferEjection: ejectionPointDirection: [{ejectionPointDirection.x}, {ejectionPointDirection.y}, {ejectionPointDirection.z}]");
            ...
            if (idealBurnUT - burnUT > o.period / 2 || burnUT < UT)
            {
                burnUT += o.period;
                if (burnUT < UT)
                    FlightPlanPlugin.Logger.LogDebug("Adding 1 orbital period because burnUT < UT");
                else
                    FlightPlanPlugin.Logger.LogDebug("Adding 1 orbital period because idealBurnUT - burnUT > o.period / 2");
        ...
        public static (Vector3d dv, double dt) DeltaVAndTimeForMoonReturnEjection(PatchedConicsOrbit o, double ut, double targetPrimaryRadius)
        {
            ...
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForMoonReturnEjection: moonR0 = {moonR0:N3} m");
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForMoonReturnEjection: moonV0 = {moonV0:N3} = {moonV0.magnitude:N3} m/s");
            double moonSOI = moon.sphereOfInfluence;
            (V3 r0, V3 v0) = o.RightHandedStateVectorsAtUT(ut);
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForMoonReturnEjection: r0 = {r0:N3} m");
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForMoonReturnEjection: v0 = {v0:N3} = {v0.magnitude:N3} m/s");
        ...
        {
            Vector3d hohmannDV;
            FlightPlanPlugin.Logger.LogDebug($"DeltaVAndTimeForHohmannLambertTransfer: Calling DeltaVAndTimeForHohmannTransfer");
            (hohmannDV, burnUT, _, _) = DeltaVAndTimeForHohmannTransfer(o, target, UT);
        ...
        private static PatchedConicsOrbit CreateOrbit() => new PatchedConicsOrbit(Game.UniverseModel);
            ...
            out PatchedConicsOrbit intercept)
        {
            ...
            FlightPlanPlugin.Logger.LogInfo($"burnVec: {burnVec:N3} = {burnVec.magnitude}");
