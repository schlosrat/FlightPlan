...
namespace MuMech
{
    ...
    public class MechJebModuleLandingPredictions : ComputerModule
    {
        ...
        public ManeuverNodeData aerobrakeNode = null;
        ...
        protected void StartSimulation(bool addParachuteError)
        {
            ...
            PatchedConicsOrbit patch = GetReenteringPatch() ?? orbit;
            ...
            ReentrySimulation sim = ReentrySimulation.Borrow(patch, patch.StartUT, simVessel, simCurves, descentSpeedPolicy, decelEndAltitudeASL, 0, parachuteMultiplierForThisSimulation, altitudeOfPreviousPrediction, addParachuteError, dt, Time.fixedDeltaTime, maxOrbits, noSkipToFreefall);
        ...
        protected PatchedConicsOrbit GetReenteringPatch()
        {
            PatchedConicsOrbit patch = orbit;
            ...
            do
            {
                ...
                PatchedConicsOrbit nextPatch = orbit; // FIX ME!  vessel.GetNextPatch(patch, aerobrakeNode);
                if (patch.Periapsis < reentryRadius)
                {
        ...
        //        {
        //            //Compute the node dV:
        //            PatchedConicsOrbit preAerobrakeOrbit = GetReenteringPatch();
        ...
        //            PatchedConicsOrbit postAerobrakeOrbit = MuUtils.OrbitFromStateVectors(r.WorldAeroBrakePosition(), r.WorldAeroBrakeVelocity(), r.Body, r.AeroBrakeUT);

        //            Vector3d dV = OrbitalManeuverCalculator.DeltaVToChangeApoapsis(preAerobrakeOrbit, UT, postAerobrakeOrbit.Apoapsis);
