...
namespace MuMech
{
    ...
    {
        // Input values
        private PatchedConicsOrbit _inputInitialOrbit;
        ...
        private readonly PatchedConicsOrbit _initialOrbit = new(GameManager.Instance.Game.UniverseModel);
        ...
        public static ReentrySimulation Borrow(PatchedConicsOrbit initialOrbit, double ut, SimulatedVessel vessel, SimCurves simcurves,
        ...
        {
            ReentrySimulation sim = _pool.FetchInstance();
        ...
        private void Init(PatchedConicsOrbit initialOrbit, double ut, SimulatedVessel vessel, SimCurves simcurves, IDescentSpeedPolicy descentSpeedPolicy,
            ...
            bool multiplierHasError, double dt, double minDT, double maxOrbits, bool noSKiptoFreefall)
        {
            ...
            CelestialBodyComponent body = initialOrbit.referenceBody;
            _bodyHasAtmosphere          = body.hasAtmosphere;
            ...
            _maxDragGees    = 0;
            _deltaVExpended = 0;
            _trajectory     = ListPool<AbsoluteVector>.Instance.FetchInstance();
        ...
        public Result RunSimulation()
        {
            ...
            finally
            {
                if (_trajectory != _result.Trajectory)
                    ListPool<AbsoluteVector>.Instance.ReleaseInstance(_trajectory);
                _vessel.Release();
                _simCurves.Release();
            }
        ...
        private bool OrbitReenters(PatchedConicsOrbit initialOrbit)
        {
            return initialOrbit.Periapsis < _decelRadius || initialOrbit.Periapsis < _aerobrakedRadius;
        }
        ...
        private void AdvanceToFreefallEnd(PatchedConicsOrbit initialOrbit)
        {
        ...
        private double FindFreefallEndTime(PatchedConicsOrbit initialOrbit)
        {
        ...
        private bool FreefallEnded(PatchedConicsOrbit initialOrbit, double ut)
        {
        ...
        private void BS34Step()
        {
            ...
            do
            {
                ...
                double speedOfSound = AeroUtilities.GetSoundSpeed(_mainBody.atmosphereAdiabaticIndex, Pressure(xForChuteSim), airDensity);
        ...
        private Vector3d TotalAccel(Vector3d pos, Vector3d vel, bool record = false)
        {
            ...
            double speedOfSound = AeroUtilities.GetSoundSpeed(_mainBody.atmosphereAdiabaticIndex, pressure, airDensity);
        ...
        private double AirDensity(Vector3d pos, double altitude)
        {
            ...
            return _mainBody.GetDensity(pressure, temp);
        }

        private double StaticPressure(double altitude)
        {
            if (!_mainBody.hasAtmosphere)
                return 0;

            if (altitude >= _mainBody.atmosphereDepth)
                return 0;

            if (_mainBody.useAtmospherePressureCurve)
            {
                if (_mainBody.isAtmospherePressureCurveNormalized)
        ...
        {
            if (!_mainBody.hasAtmosphere)
                return PhysicsSettings.SpaceTemperature;

            if (altitude > _mainBody.atmosphereDepth)
                return PhysicsSettings.SpaceTemperature;

            Vector3 up = position.normalized;
            float polarAngle = Mathf.Acos(Vector3.Dot(_mainBody.coordinateSystem.up.vector, up)); // was: _mainBody.bodyTransform.up
            ...
            // FpUiController.Kerbol.Position.localPosition
            Vector3 sunVector = (FpUiController.Kerbol.Position.localPosition - position + _mainBody.Position.localPosition).normalized;
            Vector3 bodyUp = _mainBody.coordinateSystem.up.vector; // was: _mainBody.bodyTransform.up
            ...
            double temperature;
            if (!_mainBody.useAtmosphereTemperatureCurve)
            {
                temperature = _mainBody.atmosphereTemperatureSeaLevel - _mainBody.atmosphereTemperatureLapseRate * altitude;
            }
            else
            {
                temperature = !_mainBody.isAtmosphereTemperatureCurveNormalized
        ...
        {
            double newtonianTemperatureFactor = velocity * PhysicsSettings.NewtonianTemperatureFactor;
            double convectiveMachLerp =
                Math.Pow(
                    UtilMath.Clamp01((mach - PhysicsSettings.NewtonianMachTempLerpStartMach) /
                                     (PhysicsSettings.NewtonianMachTempLerpEndMach - PhysicsSettings.NewtonianMachTempLerpStartMach)),
                    PhysicsSettings.NewtonianMachTempLerpExponent);
            if (convectiveMachLerp > 0)
            {
                double machTemperatureScalar =
                    PhysicsSettings.MachTemperatureScalar * Math.Pow(velocity, PhysicsSettings.MachTemperatureVelocityExponent);
            ...
            return newtonianTemperatureFactor * _mainBody.shockTemperatureMultiplier;
        }
        ...
        public void Log(Vector3d pos, Vector3d vel)
        {
            if (_t > _nextLog)
            {
                ...
                double speedOfSound = AeroUtilities.GetSoundSpeed(_mainBody.atmosphereAdiabaticIndex, Pressure(pos), AirDensity(pos, altitude));
                float mach = Mathf.Min((float)(airVel.magnitude / speedOfSound), 50f);
        ...
        public class Result
        {
            ...
            public CelestialBodyComponent  Body;
            ...
            public PatchedConicsOrbit InputInitialOrbit;
            ...
            {
                if (Trajectory != null)
                    ListPool<AbsoluteVector>.Instance.ReleaseInstance(Trajectory);
                Exception = null;
                _pool.ReleaseInstance(this);
            }
            ...
            {
                Result result = _pool.FetchInstance();
                return result;
            }
            ...
            {
                return WorldEndPosition() - Body.Position.localPosition;
            }
            ...
            public PatchedConicsOrbit EndOrbit()
            {
                return MuUtils.OrbitFromStateVectors(WorldEndPosition(), WorldEndVelocity(), Body.SimulationObject.transform.celestialFrame, Body, EndUT);
            }
            ...
            public PatchedConicsOrbit AeroBrakeOrbit()
            {
                return MuUtils.OrbitFromStateVectors(WorldAeroBrakePosition(), WorldAeroBrakeVelocity(), Body.SimulationObject.transform.celestialFrame, Body, EndUT);
            }

            public List<Vector3d> WorldTrajectory(double timeStep, bool world = true)
            {
                List<Vector3d> ret = ListPool<Vector3d>.Instance.FetchInstance();
            ...
            public double GetOvershoot(double targetLatitude, double targetLongitude)
            {
                ...
                Vector3d end = Body.GetWorldSurfacePosition(EndPosition.Latitude, EndPosition.Longitude, 0, Body.coordinateSystem) - Body.Position.localPosition;
                Vector3d target = Body.GetWorldSurfacePosition(targetLatitude, targetLongitude, 0, Body.coordinateSystem) - Body.Position.localPosition;
                Vector3d start = Body.GetWorldSurfacePosition(StartPosition.Latitude, StartPosition.Longitude, 0, Body.coordinateSystem) - Body.Position.localPosition;
    ...
    public class ReferenceFrame
    {
        ...
        public void UpdateAtCurrentTime(CelestialBodyComponent body)
        {
            ...
            _epoch            = GameManager.Instance.Game.UniverseModel.UniverseTime;
            _referenceBody    = body;
        }
        ...
        {
            return _referenceBody.Position.localPosition + WorldVelocityAtCurrentTime(absolute);
        }

        public Vector3d BodyPositionAtCurrentTime(AbsoluteVector absolute)
        {
            return _referenceBody.Position.localPosition + absolute.Radius * _referenceBody.GetSurfaceNVector(absolute.Latitude, absolute.Longitude);
        }
