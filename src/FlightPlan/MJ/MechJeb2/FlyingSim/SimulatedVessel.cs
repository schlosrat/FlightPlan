...
namespace MuMech
{
    public class SimulatedVessel
    {
        ...
        public static SimulatedVessel Borrow(VesselComponent v, ReentrySimulation.SimCurves simCurves, double startTime, int limitChutesStage)
        {
            SimulatedVessel vessel = pool.FetchInstance(); // was (KSP1): Borrow()
            vessel.Init(v, simCurves, startTime, limitChutesStage);
            return vessel;
        }

        private void Init(VesselComponent v, ReentrySimulation.SimCurves _simCurves, double startTime, int limitChutesStage)
        {
            totalMass = 0;

            List<PartComponent> oParts = v.SimulationObject.PartOwner.Parts.ToList();
            count = oParts.Count;
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                //{
                //    var mp = oParts[i].Modules[j] as Module_Parachute;
                //    if (mp != null && v.mainBody.hasAtmosphere)
                //    {
                //        special       = true;
        ...
        public Vector3d Drag(Vector3d localVelocity, double dynamicPressurekPa, float mach)
        {
            ...
            double dragFactor = dynamicPressurekPa * PhysicsSettings.DragCubeMultiplier * PhysicsSettings.DragMultiplier;
