    ...
    public class ReturnFromMoon
    {
        ...
        private (V3 V, double dt, V3 vinf) SolutionToManeuver(double[] x)
        {
            ...
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.SolutionToManeuver: dot(rsoi, vsoi): {V3.Dot(rsoi, vsoi)}");
        ...
        private (V3 dv, double dt, double tt1, V3 rsoi, double tt2, V3 rf) Guess(double inc, bool type2, int periodOffset, bool full = false)
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                if (full)
                {
                    ...
                    FlightPlanPlugin.Logger.LogInfo(
                        $"Guessed return ellipse: sma:{sma3 * planetScale.LengthScale:N3} ecc:{ecc3:N3} inc:{Rad2Deg(inc3):N3} lan:{Rad2Deg(lan3):N3} argp:{Rad2Deg(argp3):N3} tanom:{Rad2Deg(tanom3):N3}");

                    FlightPlanPlugin.Logger.LogInfo(
                    	$"vinf fed to hyperbolic guesser: {(vsoiPlanet - moonVsoi) * moonToPlanetScale.VelocityScale * moonScale.VelocityScale:N3}");
                }
                ...
                if (full)
                {
                    ...
                    FlightPlanPlugin.Logger.LogInfo(
                    	$"dt guess = {dt}, tt1 guess = {tt1} tt2 guess = {tt2}");
                }
            }
        ...
        private (double inc, bool type2) GuessOptimizer(int periodOffset)
        {
            ...
            Scale moonScale = _args.MoonScale;
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.GuessOptimizer: imin = {Rad2Deg(imin):N3} imincost = {imincost * moonScale.VelocityScale:N3} type2 = {type2}");
            return (imin, type2);
        }
        ...
        private double[] GenerateInitialGuess(int periodOffset)
        {
            ...
            sw.Stop();
            FlightPlanPlugin.Logger.LogInfo(
            	$"initial guess generation took {sw.ElapsedMilliseconds}ms");
            ...
            bool optguard = false)
        {
            ...
            FlightPlanPlugin.Logger.LogInfo(
            	$"optimization took {sw.ElapsedMilliseconds}ms: {rep.iterationscount} iter, {rep.nfev} fev");
            ...
            for (int i = 0; i < x2.Length; i++)
                FlightPlanPlugin.Logger.LogInfo(
                	$"x[{i}]: {x2[i]} - {x[i]} = {x2[i] - x[i]} ({100 * (x2[i] - x[i]) / x2[i]}%)");
            ...
            if (rep.terminationtype < 0)
                throw new Exception(
                    $"ReturnFromMoon.ManeuverScaled(): SLP solver terminated abnormally: {rep.terminationtype}"
                );

            if (rep.nlcerr > 1e-2)
                throw new Exception(
                    $"ReturnFromMoon.ManeuverScaled() no feasible solution found, constraint violation: {rep.nlcerr}");
        ...
        {
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.Maneuver: centralMu = {centralMu:N3}, moonMu = {moonMu:N3}, moonSOI = {moonSOI:N3}, peR = {peR:N3}, inc = {inc:N3}");
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.Maneuver: moonR0 = {moonR0:N3} m");
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.Maneuver: moonV0 = {moonV0:N3} = {moonV0.magnitude:N3} m/s");
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.Maneuver: r0 = {r0:N3} m");
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.Maneuver: v0 = {v0:N3} = {v0.magnitude:N3} m/s");
            ...
            */
            ...
            sw.Stop();
            FlightPlanPlugin.Logger.LogInfo(
            	$"total calculation took {sw.ElapsedMilliseconds}ms");
        ...
        private void LogStuff1(Scale moonScale)
        {
            ...
            FlightPlanPlugin.Logger.LogInfo(
                $"Secondary celestial orbit: sma:{sma * planetScale.LengthScale:N3} ecc:{ecc:N3} inc:{Rad2Deg(inc):N3} lan:{Rad2Deg(lan):N3} argp:{Rad2Deg(argp):N3} tanom:{Rad2Deg(tanom):N3}");

            (sma, ecc, inc, lan, argp, tanom, _) =
                Astro.KeplerianFromStateVectors(1.0, r0, v0);

            FlightPlanPlugin.Logger.LogInfo(
                $"Parking orbit around Secondary: sma:{sma * moonScale.LengthScale:N3} ecc:{ecc:N3} inc:{Rad2Deg(inc):N3} lan:{Rad2Deg(lan):N3} argp:{Rad2Deg(argp):N3} tanom:{Rad2Deg(tanom):N3}");
        }
        ...
        private void LogStuff2(V3 dv, double dt, V3 vinf)
        {
            ...
            FlightPlanPlugin.Logger.LogInfo(
                $"Ejection orbit around Secondary: sma:{sma * moonScale.LengthScale:N3} ecc:{ecc:N3} inc:{Rad2Deg(inc):N3} lan:{Rad2Deg(lan):N3} argp:{Rad2Deg(argp):N3} tanom:{Rad2Deg(tanom):N3}");

            (sma, ecc, inc, lan, argp, tanom, _) =
                Astro.KeplerianFromStateVectors(1.0, r3, v3);

            FlightPlanPlugin.Logger.LogInfo(
                $"Return orbit around Primary: sma:{sma * planetScale.LengthScale:N3} ecc:{ecc:N3} inc:{Rad2Deg(inc):N3} lan:{Rad2Deg(lan):N3} argp:{Rad2Deg(argp):N3} tanom:{Rad2Deg(tanom):N3}");

            double peR = Astro.PeriapsisFromStateVectors(1.0, r3, v3);

            FlightPlanPlugin.Logger.LogInfo(
            	$"vinf: {vinf * moonScale.VelocityScale:N3} PeR: {peR * planetScale.LengthScale:N3}");

            FlightPlanPlugin.Logger.LogInfo(
                $"deltaV: {dv * moonScale.VelocityScale}/{dv.magnitude * moonScale.VelocityScale:N3} dt: {dt * moonScale.TimeScale} (solved)");
            ...
            if (Astro.EccFromStateVectors(1.0, r0, v0) < 1 && Astro.ApoapsisFromStateVectors(1.0, r0, v0) < moonSOI)
            {
                ...
                FlightPlanPlugin.Logger.LogInfo(
                    $"deltaV: {dv2 * moonScale.VelocityScale}/{dv2.magnitude * moonScale.VelocityScale:N3} dt: {dt2 * moonScale.TimeScale} (analytic validation)");
            }
        }
            ...
            double peR, double inc, bool optguard = false)
        {
            ...
            {
                FlightPlanPlugin.Logger.LogInfo(
                	$"ReturnFromMoon.NextManeuver: ecc = {ecc}");
                (dv, dt) = Maneuver(centralMu, moonMu, moonR0, moonV0, moonSOI, r0, v0, peR, inc, 0, optguard: optguard);
                FlightPlanPlugin.Logger.LogInfo(
                	$"ReturnFromMoon.NextManeuver: dv = {dv:N3} = {dv.magnitude:N3} m/s, dt = {dt:N3}");
            }
            else
            {
                ...
                double dtmax = period;
                FlightPlanPlugin.Logger.LogInfo(
                	$"ReturnFromMoon.NextManeuver: ecc = {ecc:N3}, period = {period:N3}, periodOffset = {periodOffset}, dtmin = {dtmin:N3}, dtmax = {dtmax:N3}");
                while (true)
                {
                    ...
                    if (dt <= 1e-4)
                    {
                        ...
                        dtmax        += period;
                        FlightPlanPlugin.Logger.LogInfo(
                        	$"ReturnFromMoon.NextManeuver: dt <= 1e-4: periodOffset = {periodOffset}, dtmax = {dtmax:N3}");
                    }
                    else if (NearlyEqual(dt, dtmax, 1e-4) || dt >= dtmax)
                    {
                        dtmax += period;
                        FlightPlanPlugin.Logger.LogInfo(
                        	$"ReturnFromMoon.NextManeuver: dtmax = {dtmax:N3}");
                    }
                    else
                    {
                        break;
            ...
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.NextManeuver: r1 = {r1:N3}, v1 = {v1:N3} = {v1.magnitude:N3} m/s");
            double tt1 = Astro.TimeToNextRadius(moonMu, r1, v1 + dv, moonSOI);
            (V3 r2, V3 v2)         = Shepperd.Solve(moonMu, tt1, r1, v1 + dv);
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.NextManeuver: r2 =[{r2:N3}, v2 = {v2:N3} = {v2.magnitude:N3} m/s");
            (V3 moonR2, V3 moonV2) = Shepperd.Solve(centralMu, dt + tt1, moonR0, moonV0);
            FlightPlanPlugin.Logger.LogInfo(
            	$"ReturnFromMoon.NextManeuver: moonR2 = {moonR2:N3}, moonV2 = {moonV2:N3} = {moonV2.magnitude:N3} m/s");
