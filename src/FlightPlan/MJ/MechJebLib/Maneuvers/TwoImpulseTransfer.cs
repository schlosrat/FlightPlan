    ...
    public static class TwoImpulseTransfer
    {
            ...
            bool fixedTime = false, bool optguard = false)
        {
            ...
            FlightPlanPlugin.Logger.LogInfo(
            	$"TwoImpulseTransfer.NextManeuver: synodicPeriod = {synodicPeriod}, mu = {mu}");
            FlightPlanPlugin.Logger.LogInfo(
            	$"TwoImpulseTransfer.NextManeuver: r1 = {r1:N3}, v1 = {v1:N3} = {v1.magnitude:N3} m/s");
            FlightPlanPlugin.Logger.LogInfo(
            	$"TwoImpulseTransfer.NextManeuver: r2 = {r2:N3}, v2 = {v2:N3} = {v2.magnitude:N3} m/s");
            FlightPlanPlugin.Logger.LogInfo(
            	$"TwoImpulseTransfer.NextManeuver: lagTime = {lagTime}, fixedTime = {fixedTime}, Coplanar = {coplanar}, Rendezvous = {rendezvous}, Capture = {capture}, optguard = {optguard}");

            if (fixedTime)
            {
                FlightPlanPlugin.Logger.LogInfo(
                	$"TwoImpulseTransfer.NextManeuver: Returning ManeuverInternal (fixedTime = {fixedTime})");
            ...
            {
                FlightPlanPlugin.Logger.LogInfo(
                	$"TwoImpulseTransfer.NextManeuver: Calling ManeuverInternal (iter = {iter})");
