...
#nullable restore
        ...
        public static int HoursPerDay => SessionManager.KERBIN_TIME ? 6 : 24;
        public static int DaysPerYear => SessionManager.KERBIN_TIME ? 426 : 365;
        ...
        {
            VesselComponent _activeVessel = GameManager.Instance?.Game?.ViewController?.GetActiveVehicle(true)?.GetSimVessel(true);
            double a = (_activeVessel.mainBody.transform.Position.localPosition - From).magnitude;
            double b = (_activeVessel.mainBody.transform.Position.localPosition - To).magnitude;
            double c = Vector3d.Distance(From, To);
            double ang = Math.Acos((a * a + b * b - c * c) / (2f * a * b));
            return ang * _activeVessel.mainBody.radius;
        }

        public static double FromToETA(Vector3 From, Vector3 To, double Speed = 0)
        {
            VesselComponent _activeVessel = GameManager.Instance?.Game?.ViewController?.GetActiveVehicle(true)?.GetSimVessel(true);
            return ArcDistance(From, To) / (Speed > 0 ? Speed : _activeVessel.horizontalSrfSpeed);
        }
