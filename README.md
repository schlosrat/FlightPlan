# Flight Plan
![Flight Plan Banner Image](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-banner-3.png)
...
![Flight Plan Celestial Target Selection Menu](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-GUI-9.png)
...
![Flight Plan Burn Time Option Menu](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-GUI-10.png)
...
![Flight Plan Main GUI](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/OSM_50.png)
* **Circularize**
...
![Flight Plan Main GUI](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/TRM_50_Ship2Ship.png) **Only available if a target vessel or part is selected and both active vessel and target are in the same SOI**
...
![Flight Plan Main GUI](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/TRM_50_Ship2Celestial.png) **Only available if a celestial target is selected and both active vessel and target are in the same SOI**
...
![Flight Plan Main GUI](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/OTM_50_Moon.png) **Only available when in orbit about a moon**
...
![Flight Plan Main GUI](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/OTM_50_Planet.png) **Only available when orbiting a planet targeting another planet**
...
![Flight Plan Main GUI](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/ROM_50.png)
...
* Warnings and Cautions are shown in **Yellow** indicating a node was generated, but the % error between the requested outcome and the actual outcome is above the threshold for a small % error. you should inspect it carefully first and may need to modify it. If auto-launch is enabled for Maneuver Node Controller, and that mod is installed, then it will be automatically brought up if it's not already up.
* Failures are shown in **Red** indicating that either no node has been generated, or the node quality does not fall below the threshold for a large % error. If auto-launch is enabled for Maneuver Node Controller, and that mod is installed, then it will be automatically brought up if it's not already up. If no node is generated there will be a message stating why.
...
![Flight Plan GUI Examples](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-GUIs.png)
...
![Flight Plan Configuration Parameters](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Config_Menu.png)
...
![Flight Plan Future GUI](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-GUI-4.png)
...
![Flight Plan: Match Planes with Minmus 1](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-1.png)
...
![Flight Plan: Match Planes with Minmus 2](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-2.png)

Here we can see K2-D2's status indicates we're warping to the burn. The Flight Plan status is unchanged.
![Flight Plan: Match Planes with Minmus 3](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-3.png)

Here we can see K2-D2's status indicates we're executing the burn. The Flight Plan status is unchanged.
![Flight Plan: Match Planes with Minmus 4](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-4.png)
...
![Flight Plan: Match Planes with Minmus 5](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-5.png)
...
![Flight Plan: Match Planes with Minmus 6](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-6.png)

Here we can see K2-D2 has been commanded to execute the node and we're warping to the starting point for the burn.
![Flight Plan: Match Planes with Minmus 7](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-7.png)

Here we can see K2-D2 executing the transfer burn.
![Flight Plan: Match Planes with Minmus 8](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-8.png)
...
![Flight Plan: Match Planes with Minmus 8a](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-8a%20-%20overshoot.png)
...
![Flight Plan: Match Planes with Minmus 9](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-9.png)
...
![Flight Plan: Match Planes with Minmus 10](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-10.png)
...
![Flight Plan: Match Planes with Minmus 11](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-11.png)
...
![Flight Plan: Match Planes with Minmus 12](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-12.png)
...
![Flight Plan: Match Planes with Minmus 13](https://raw.githubusercontent.com/schlosrat/FlightPlan/master/media/FP-Action-13.png)
...
* **SetNewPe(burnUT, newPeR, burnOffsetFactor)**: Calling this method will create a maneuver node at the specified time to set the vessel's Periapsis (measured from the center of the body, not altitude above the surface).
* **SetNewAp(burnUT, newApR, burnOffsetFactor)**: Calling this method will create a maneuver node at the specified time to set the vessel's Apoapsis (measured from the center of the body, not altitude above the surface).
* **Ellipticize(burnUT, newApR, newPeR, burnOffsetFactor)**: Calling this method will create a maneuver node at the specified time to set the vessel's Apoapsis and Periapsis (both measured from the center of the body, not altitude above the surface).
...
* **InterceptTgt(burnUT, deltaUT, burnOffsetFactor)**: Calling this method will create a maneuver node at the specified time to intercept the currently selected target at the time of deltaUT from now.
* **CourseCorrection(burnUT, burnOffsetFactor)**: Calling this method will create a maneuver node at the *optimal time* to fine-tune the trajectory to intercept the currently selected target. This method may be useful after executing a Hohmann Transfer or Moon Return maneuver node. Calling it before node execution is sub-optimal and not advised. *NOTE*: The supplied burnUT parameter presently has no effect.
* **MoonReturn(burnUT, targetPe, burnOffsetFactor)**: Calling this method will create a maneuver node at the *optimal time* for a Hohmann Transfer to return the active vessel from a moon to the planet the moon is orbiting in a transfer orbit that gives the targetPe when arriving at the parent planet. *NOTE*: The supplied burnUT parameter presently has no effect.
...
* **NextTimeOfRadius(UT, Radius):** This is the next time *after* UT at which the active vessel will have the radius specified. Returns -1 if the specified radius is not possible given the current orbit.
